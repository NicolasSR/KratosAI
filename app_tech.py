...
import kratos_io
import clustering
import networks.gradient_shallow_tech as gradient_shallow_ae
...
if __name__ == "__main__":
    ...
    insults = [
        # "training/insults/result_80000.npy",
        # "training/insults/result_90000.npy",
        "training/insults/result_100000.npy",
        # "training/insults/result_110000.npy",
        # "training/insults/result_120000.npy",
    ]
    ...
    if config["train_model"]:
        autoencoder.w = 0.95
        kratos_network.train_network(autoencoder, I.T, (SReduced.T,R,F), len(data_inputs), 2000)
    ...
    print(f"NNM norm error : {np.linalg.norm(SP1.T[:-1]-ST.T[1:])/np.linalg.norm(ST.T[1:])}")
    print(f"SVD norm error : {np.linalg.norm(SPri.T[1:]-S.T[1:])/np.linalg.norm(S.T[1:])}")
    ...
    if config["print_results"]:
        ...
        kratos_io.print_results_to_gid(model_part, S.T[1:], SP1.T[:-1])
