...
if __name__ == "__main__":
    ...
    def calculate_R_norm_error(S_input, R_true, F_true):
        ...
        print('Residual. Mean rel L2 error:', l2_error)
        print('Residual. Rel Forb. error:', forb_error)

    def calculate_X_norm_error(S_input):
        S_pred_norm=autoencoder(S_input).numpy()
        S_pred = data_normalizer.denormalize_data(S_pred_norm)
        l2_error=mean_relative_l2_error(S_input,S_pred)
        forb_error=relative_forbenius_error(S_input,S_pred)
    ...
    def calculate_SVD_error():
        M, N = S_orig_train.shape
        u,s,_ = np.linalg.svd(S_orig_train)
        print(s)
        u = u[:,:]
        S_svd=u@u.T@S_orig_train

        R_errors_vec=[]
        X_errors_vec=[]
        for i in range(S_svd.shape[1]):
            R_errors_vec.append(autoencoder.manual_r_loss_svd_test(S_svd[:,i], R_train[i,:]))
            X_errors_vec.append(np.linalg.norm(S_svd[:,i]-S_orig_train[:,i]))
        print(np.mean(R_errors_vec))
        print(np.mean(X_errors_vec))
    ...
    def print_x_and_r_vecs(sample_id):
        x_orig=np.expand_dims(S_orig[sample_id], axis=0)
        x_norm=np.expand_dims(S[sample_id], axis=0)
        x_pred_norm=autoencoder(np.expand_dims(S[sample_id], axis=0)).numpy()
        x_pred=data_normalizer.denormalize_data(x_pred_norm)
        ...
        plt.plot(x_norm[0], '.')
        plt.plot(x_pred_norm[0], '.')
        plt.show()

        print(r_orig[0].shape)
        print(r_pred[0].shape)
    ...
    def draw_x_error_image():
        ...
        im1 = ax1.imshow(image, extent=[1,S.shape[1],np.max(np.abs(F[:,1])),np.min(np.abs(F[:,1]))], interpolation='none')
