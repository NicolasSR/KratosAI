...
if __name__ == "__main__":
    ...
    print("SReduced shape: ", SReduced.shape)

    kratos_network.calculate_data_limits(SReduced)
    print('Min value in input:', kratos_network.data_min)
    print('Max value in input:', kratos_network.data_max)
    print('Real min:', np.min(SReduced))
    print('Real max:', np.max(SReduced))

    S_train_T, S_test_T, R_train, R_test, F_train, F_test = train_test_split(SReduced.T, R, F, test_size=test_ratio, shuffle=True)
    ...
    if config["train_model"]:
        ...
        history = kratos_network.train_network(autoencoder, SRedNorm_train, R_train, 100)
