...
class Conv2DResidualMainAEModel(keras.Model):

    def __init__(self,*args,**kwargs):
        super(Conv2DResidualMainAEModel,self).__init__(*args,**kwargs)
        self.w=0
        self.lam=0
    ...
    def get_jacobians(self, trainable_vars, x_true, b_true):
        with tf.GradientTape(persistent=True) as tape_d:
            ...
            loss_orth=self.norm_loss(tf.matmul(b_true,trainable_vars[0]))
    ...
    def test_step(self, data):
        ...
        for sample_id in range(batch_len):
            ...
            loss_orth=self.norm_loss(tf.matmul(b_true,self.trainable_variables[0]))
            total_loss_orth+=loss_orth
