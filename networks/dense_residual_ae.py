...
class DenseResidualAEModel(keras.Model):

    def __init__(self,*args,**kwargs):
        super(DenseResidualAEModel,self).__init__(*args,**kwargs)
    ...
    def get_jacobians(self, trainable_vars, x_true):
        with tf.GradientTape(persistent=True) as tape_d:
            ...
            x_pred_denorm = self.data_normalizer.denormalize_data_tf(x_pred)
    ...
    def train_step(self,data):
        ...
        if w == 1:
            ...
            for layer in jac_u:
                ...
                # grad_loss_r=tf.matmul(tf.multiply(err_r,self.residual_weights_vec),pre_grad)*(-2)
    ...
    def get_r_array(self, samples, F_true):
        samples_denorm = self.data_normalizer.denormalize_data(samples)
        b_list=[]
...
class Dense_Residual_AE():

    def __init__(self):
        return None

    def squared_error_metric(_,y_true,y_pred):
        return (y_true - y_pred) ** 2
    ...
    def define_network(self, input_data, ae_config):
        ...
        hid_sizes_encoder = ae_config["hidden_layers"][0]
        hid_sizes_decoder = ae_config["hidden_layers"][1]
        ...
        for layer_size in hid_sizes_encoder:
            ...
            encoder_out = tf.keras.layers.ELU()(encoder_out)
        ...
        self.autoenco = DenseResidualAEModel(model_input, self.decoder_model(self.encoder_model(model_input)), name='Autoencoder')
        
        # self.autoenco.compile(optimizer=tf.keras.optimizers.Adam(amsgrad=True), run_eagerly=True, metrics=[self.squared_error_metric])
        self.autoenco.compile(optimizer=tf.keras.optimizers.experimental.AdamW(), run_eagerly=True, metrics=[self.squared_error_metric])
